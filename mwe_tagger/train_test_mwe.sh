...
" 
	python -c "$SCRIPT" cmwe/corpus.mwe train.sentids > cmwe/train.mwe
	python -c "$SCRIPT" cmwe/corpus.mwe test.sentids > cmwe/test.mwe

	# convert to tags

	python src/mwe2tags.py cmwe/train.mwe > cmwe/train.tags
	python src/mwe2tags.py cmwe/test.mwe > cmwe/test.tags
...
python src/main.py --mwe --YY tagsets/bio2g --defaultY O --train cmwe/train.syspos.tags --test-predict cmwe/test.syspos.tags --iters 3 --debug --save mwe.model --bio NO_SINGLETON_B --clusters --cluster-file mwelex/yelpac-c1000-m25.gz --lex mwelex/{semcor_mwes,wordnet_mwes,said,phrases_dot_net,wikimwe,enwikt}.json /dev/null --includeLossTerm --costAug 100  > testpredictions.syspos.tags
# the /dev/null was supposed to be ignored, but it does have a small effect

# - convert predictions to .mwe

python src/tags2mwe.py testpredictions.syspos.tags > testpredictions.syspos.mwe

# - evaluate

python src/tags2mwe.py cmwe/test.tags > cmwe/test.withtags.mwe

python src/mweval.py --default-strength strong cmwe/test.withtags.mwe testpredictions.syspos.mwe > testpredictions.eval
