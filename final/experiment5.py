...
class SpeechSTIIExperimentRunner:
    ...
    def mask_waveform(self, waveform, sample_rate, mask):
        ...
        for idx, m in enumerate(mask):
            if m:
                start = int(idx * 0.02 * sample_rate)
                end = int((idx+1) * 0.02 * sample_rate)
    ...
    def interaction_value_stii(self, waveform, sample_rate, phonemes_df):
        ...
        perms = self.generate_perm(self.NUMBER_OF_PERM, nframes)
        ...
        for l in range(nframes):
            for r in range(l+1, min(nframes, l+self.RIGHT_MAX+1)):
                ...
                templ = phonemes_df[(l*0.02 >= phonemes_df['start']) & (l*0.02 <= phonemes_df['end'])]
                ...
                tempr = phonemes_df[(r*0.02 >= phonemes_df['start']) & (r*0.02 <= phonemes_df['end'])]
...
def run_sample(number):
    try:
        ...
        output_csv_path = f"../speech_data/stii_outputs_fix/stii_{number}.csv"
...
os.makedirs("../speech_data/stii_outputs_fix/", exist_ok=True)
from tqdm import tqdm
